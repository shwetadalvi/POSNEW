...
public class SupplierSpinnerAdapter<T> extends ArrayAdapter<T> {

    private final Context context;
    private List<T> spinnerItemList = new ArrayList<>();

    public SupplierSpinnerAdapter(Context context, int textViewResourceId, List<T> spinnerItemList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (spinnerItemList.get(position) instanceof Supplier) {
            mViewHolder.item.setText(((Supplier) spinnerItemList.get(position)).getSupplierName());
        }
        return convertView;
    }
